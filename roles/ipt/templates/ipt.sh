...
{% if ( ssh_ports is defined ) and ( SSH_PUB is defined ) %}
########## SSH 
ssh_port={{ssh_ports}}

# iptables -A INPUT -p tcp -m multiport --dports ${ssh_port} -j fail2ban-sshd
# iptables -A INPUT -m set --match-set fail2ban-sshd src -j RETURN
